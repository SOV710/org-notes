#+title: Building a Tractyl Manuform Ergonomic Keyboard: A Complete Guide
#+author: SOV710
#+date: 2025-12-21
#+startup: showall
...
Dactyl 键盘最初由 Matthew Adereth 在 2012 年左右设计，其设计理念源于两个经典人体工学键盘:

- *Kinesis Advantage*: 拥有弯曲键井 (curved keywell) 设计，让手指自然弯曲而非平直伸展
- *ErgoDox*: 分体式设计，允许双手自然分开，减少肩部压力

Dactyl 将这两者的优点结合，创造出既有弯曲键井又分体式的键盘。随后 Tom Short 在 2015 年左右基于 Dactyl 创建了 *Dactyl Manuform*，主要改进是将拇指簇从 ManuForm 键盘 (来自 GeekHack 论坛的设计) 移植过来，并简化了外壳的打印难度 (将侧壁改为直接垂直落地)。

Tractyl Manuform 则是在 Dactyl Manuform 基础上，由 Noah Prince 在 2020 年左右添加了轨迹球集成功能，使键盘实现 "手不离键盘" 的完整输入方案。

** 人体工学设计原理

这类键盘的核心设计理念:

1. *列交错布局 (Column-stagger)*: 不同于传统键盘的行交错，键盘按列排列，符合手指长度差异
2. *弯曲键井*: 键帽呈碗状排列，减少手指伸展距离
3. *分体式*: 双手自然分开，肩膀放松
4. *拇指簇*: 充分利用拇指的灵活性，分担小指负担
5. *可调倾角*: 通过支撑脚调整键盘角度，适应不同手腕姿势
...
PMW3360 是 PixArt 公司生产的高端光学传感器，广泛应用于游戏鼠标:

- *最高 DPI*: 12,000 (可调)
- *最高追踪速度*: 250 IPS (inches per second)
- *最高加速度*: 50g
- *帧率*: 12,000 FPS
- *轮询率*: 1000Hz (1ms 响应时间)
- *特性*: 无硬件加速，1:1 完美追踪，无角度修正

这使得 PMW3360 成为集成到键盘中的理想选择: 它提供了专业级的精度，同时功耗适中，适合 Pro Micro 驱动。
...
| Pro Micro         | 2-3  | 建议买 3 个备用         |
| TRRS 插座         | 2    | PJ-320A               |
| TRRS 线缆         | 1    | 4 芯                  |
| PMW3360 传感器    | 1    | 如果启用轨迹球          |
| 轨迹球 (34mm)     | 1    | 如果启用轨迹球          |
| 滚珠轴承           | 3    | MR63ZZ                |
| 销钉              | 3    | 3mm 直径              |
...
2. 使用万用表检查:
   - 短路: 各引脚之间不应短路
   - 通路: 按下开关时，对应的行列应导通
3. 插入测试开关，使用 QMK 测试工具或 [[https://www.keyboardtester.com/][键盘测试网站]]
4. 逐个测试所有按键位置
5. 测试轨迹球传感器 (如果有)
...
QMK (Quantum Mechanical Keyboard) 是开源的键盘固件框架，支持几乎所有自定义键盘。它的特性包括:

- *完全可编程*: 每个按键都可以自定义功能
- *层 (Layers)*: 类似 Fn 键，可以定义多个键盘布局层
- *宏*: 支持复杂的按键序列和脚本
- *鼠标键*: 用键盘模拟鼠标
- *Tap Dance*: 单击, 双击, 长按触发不同功能
- *Home Row Mods*: 在 home row 上实现修饰键
- *RGB 灯光*: 支持 RGB 背光和下光

QMK 运行在 AVR 和 ARM 微控制器上，使用 C 语言编写，编译后通过 USB 刷入键盘。
...
参考 Noah 的配置:
- [[https://github.com/noahprince22/qmk_firmware/tree/trackball/keyboards/handwired/dactyl_manuform/4x5/pmw3360][PMW3360 驱动代码]]
- [[https://github.com/noahprince22/qmk_firmware/compare/noah...noahprince22:trackball][配置 diff]]
...
2. 寻找其他项目:
   - [[https://github.com/carbonfet/dactyl-manuform-mini-keyboard][Dactyl Manuform PCB]]
...
1. *优化键位映射:* 使用 [[https://config.qmk.fm/][QMK Configurator]] 或手动编辑 keymap.c
...
- *Noah 的 QMK Fork (轨迹球):* [[https://github.com/noahprince22/qmk_firmware/tree/trackball]]
- *Dactyl 键盘社区:* [[https://github.com/adereth/dactyl-keyboard]]
- *Clojure 文档:* [[https://clojure.org/]]
- *OpenSCAD 手册:* [[https://openscad.org/documentation.html]]

* 联系和贡献

如有问题或改进建议:
- 提交 Issue 到原项目: [[https://github.com/noahprince22/tractyl-manuform-keyboard]]
